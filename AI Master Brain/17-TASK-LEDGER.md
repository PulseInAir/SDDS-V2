...
| CLIENT-JOURNEY-05 | DONE | Fix Step 3 (Invoice) to subsume Documents + Invoices & Payments + Refunds with explicit 2-window structure; persist ITR-V extraction to filing_cases | CLIENT-JOURNEY-04-R1 | ITR-V auto-extract persists ITR No. + Refund Amount to filing_cases; Step 3 explicit Window A (upload only) and Window B (charges + refund + invoice with re-upload + Draft/Final Invoice); UploadITRVStep gains compact re-upload widget; InvoiceStep exposes Draft and Issue Final actions; build+typecheck+lint pass | `1dbd7c3` |
| CLIENT-JOURNEY-05-R1 | DONE | Refinement: Remove explicit Charges confirmation step; auto-populate calculator natively in Window B concurrently with Refund and Invoice forms. | CLIENT-JOURNEY-05 | All Window B forms display concurrently. `ChargesStep` removed. `InvoiceCreateForm` receives initial refund claim. `journey-resolver` updated. Typecheck and build passed. | `f860b2b` |
