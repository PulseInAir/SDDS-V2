create table public.invoice_sequences (
  ...
  constraint invoice_sequences_pkey primary key (workspace_id, assessment_year_id),
...
create table public.invoices (
  ...
  constraint invoices_client_workspace_fk
    foreign key (workspace_id, client_id)
    references public.clients(workspace_id, id),
  constraint invoices_assessment_year_workspace_fk
    foreign key (workspace_id, assessment_year_id)
    references public.assessment_years(workspace_id, id),
  constraint invoices_case_context_fk
  ...
  constraint invoices_due_date_order
    check (issue_date is null or (due_date is not null and due_date >= issue_date)),
  constraint invoices_lifecycle_dates check (
    (
      ...
      and (
        (issue_date is null and due_date is null and issued_at is null)
        or (issue_date is not null and due_date is not null and issued_at is not null)
      )
    )
  ),
...
create unique index invoices_number_unique on public.invoices (workspace_id, invoice_number);
create unique index invoices_serial_unique on public.invoices (workspace_id, assessment_year_id, serial_number);
create index invoices_client_ay_idx
  on public.invoices (workspace_id, client_id, assessment_year_id, created_at desc)
  where archived_at is null;
create index invoices_status_due_idx
  on public.invoices (workspace_id, status, due_date)
  where archived_at is null;
create index invoices_case_idx
  on public.invoices (workspace_id, case_id)
...
create table public.invoice_items (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint invoice_items_invoice_workspace_fk
...
create index invoice_items_invoice_order_idx
  on public.invoice_items (workspace_id, invoice_id, display_order, created_at);
...
create table public.payments (
  ...
  constraint payments_invoice_workspace_fk
    foreign key (workspace_id, invoice_id)
    references public.invoices(workspace_id, id),
...
create index payments_invoice_date_idx
  on public.payments (workspace_id, invoice_id, payment_date, created_at)
  where reversed_at is null;
create index payments_recorded_by_idx
  on public.payments (recorded_by, created_at desc);

comment on table public.invoice_sequences is 'Atomic serial allocator by workspace and assessment year for invoice numbers.';
comment on table public.invoices is 'Client invoices with issue lifecycle and reconciled totals.';
comment on table public.invoice_items is 'Immutable-draft invoice line items used to derive invoice subtotal.';
comment on table public.payments is 'Partial and full invoice payments with reversible settlement history.';
