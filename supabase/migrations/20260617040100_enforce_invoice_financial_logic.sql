...
begin
  ...
  elsif new.invoice_number is distinct from old.invoice_number
     or new.serial_number is distinct from old.serial_number
     or new.workspace_id is distinct from old.workspace_id
     or new.client_id is distinct from old.client_id
     or new.case_id is distinct from old.case_id
     or new.assessment_year_id is distinct from old.assessment_year_id then
    raise exception 'invoice identity and ownership are immutable';
  end if;

  if tg_op = 'UPDATE' and new.discount_amount is distinct from old.discount_amount and old.status <> 'draft' then
  ...
  if new.status in ('issued','partially_paid','paid','overdue') then
    ...
    if new.due_date is null then new.due_date := new.issue_date + 30; end if;
    if new.issued_at is null then new.issued_at := pg_catalog.now(); end if;
    new.cancelled_at := null;
  elsif new.status = 'cancelled' then
    if new.cancelled_at is null then new.cancelled_at := pg_catalog.now(); end if;
  ...
  if new.status <> 'draft' and new.status <> 'cancelled' and new.total_amount <= 0 then
...
begin
  ...
  if tg_op = 'UPDATE' and (new.invoice_id is distinct from old.invoice_id or new.workspace_id is distinct from old.workspace_id) then
...
revoke all on function private.active_invoice_paid_amount(uuid) from public, anon, authenticated;
